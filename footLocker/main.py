...
def generate_phone_number():
    ...
    prefix = random.choice(exchange_codes)
...
async def clearElementRobust(driver, xpath):
    """Clear a text field with multiple fallback methods"""
    element = await driver.find_element(By.XPATH, xpath, timeout=20)
    
    # Try standard clear first
    await element.clear()
    
    # If field still has content, try select all + delete
    current_value = await element.get_attribute("value")
    if current_value:
        await element.click(move_to=True)
        # Select all and delete
        await element.send_keys(Keys.CONTROL + "a")
        await element.send_keys(Keys.DELETE)
        
    # Final check - if still has content, use JavaScript
    current_value = await element.get_attribute("value")
    if current_value:
        await driver.execute_script("arguments[0].value = '';", element)
...
async def create_account(emailStr):
    ...
    try:
        ...
        async with webdriver.Chrome(options=options) as driver:
            ...
            await driver.get("https://www.footlocker.com/accounts/register", wait_load=True)
            ...
            try:
                await clickElement(driver, '//*[@id="touAgreeBtn"]')
            except:
                pass
            ...
            await sendChars(driver, '//*[@id="mobileNumber"]', generate_phone_number())

            time.sleep(random.uniform(0.1, 0.2))

            await sendChars(driver, '//*[@id="postalCode"]', "97219")

            time.sleep(random.uniform(0.1, 0.2))

            if not password:
                password = "BestAcc@1234"
            ...
            await clickElement(driver, '//*[@id="FlxEnrollmentForm"]/div[2]/div/button')

            try:
                await driver.find_element(By.XPATH, '//*[@id="flxProfileInfoModalDescription"]/div/div[2]/div/div[1]/span', timeout=10)
            ...
            try:
                await driver.find_element(By.XPATH, '//*[@id="flxProfileInfoModalDescription"]/div/div[2]/div/div[3]/div[1]/button', timeout=2)
                print(green_text + "Account created: " + emailStr + reset)
                addAccount(emailStr, password, original)
            except:
                pass
            ...
            try:
                await driver.find_element(By.XPATH, '//*[@id="signInModalHeading"]', timeout=5)
                print(green_text + "Account created: " + emailStr + reset)
                addAccount(emailStr, password, original)
            except:
                try:
                    await driver.find_element(By.XPATH, '//*[@id="flxProfileInfoModalDescription"]/div/div[2]/div/div[1]/span', timeout=3)
                    print(green_text + "Account created: " + emailStr + reset)
                    addAccount(emailStr, password, original)
                except:
                    pass
                ...
                if "https://www.footlocker.com/accounts/welcome" in current:
                    print(green_text + "Account created: " + emailStr + reset)
                    addAccount(emailStr, password, original)
                else:
                    # Try new phone
                    try:
                        await driver.find_element(By.XPATH, '//*[@id="FlxEnrollmentForm"]/div[2]/div/button', timeout=3)

                        for _ in range(3):
                            print("\n-> Trying new phone number")
                            await clearElementRobust(driver, '//*[@id="mobileNumber"]')
                            ...
                            try:
                                await driver.find_element(By.XPATH, '//*[@id="signInModalHeading"]', timeout=10)
                                print(green_text + "Account created: " + emailStr + reset)
                                addAccount(emailStr, password, original)
                                return
                            except:
                        ...
                        raise Exception("Failure account creation, 3 attemps failed")
                    except:
