...
async def create_account(emailStr):
    ...
    try:
        ...
        async with webdriver.Chrome(options=options) as driver:
            ...
            await driver.get("https://www.levi.com/US/en_US/join", wait_load=True)
            ...
            time.sleep(random.uniform(5.5, 7.9))

            # Enter first name
            await sendChars(driver, '/html/body/div[1]/div/div[2]/div[1]/div/section/form/div[1]/div/div/input', random.choice(commonFirstNames))

            # Enter last name
            await sendChars(driver, '/html/body/div[1]/div/div[2]/div[1]/div/section/form/div[2]/div/div/input', random.choice(commonLastNames))

            # Enter email
            await sendChars(driver, '/html/body/div[1]/div/div[2]/div[1]/div/section/form/div[3]/div/div/input', emailStr)

            # Enter password
            if not password:
                password = MAIN_PASSWORD
            await sendChars(driver, '/html/body/div[1]/div/div[2]/div[1]/div/section/form/div[4]/div[1]/div/input', password)
            
            # Scroll down
            await driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")

            # Enter bday
            dropdownMonth = await driver.find_element(By.XPATH, '//*[@id="dropdown-2"]')
            await select(dropdownMonth, value=str(random.randint(1, 12)))

            dropdownYear = await driver.find_element(By.XPATH, '//*[@id="dropdown-3"]')
            await select(dropdownYear, value="2003")

            time.sleep(random.uniform(0.5, 1.2))

            # Click create account
            await clickElement(driver, '//*[@id="app"]/div/div[2]/div[1]/div/section/form/div[8]/button')

            # Check
            try:
                await driver.find_element(By.XPATH, '//*[@id="block-simple-headline2"]', timeout=20)
                print(green_text + "Account created: " + emailStr + reset)
                addAccount(emailStr, password, original)
            except:
                current = await driver.current_url

                if current == "https://www.levi.com/US/en_US/":
                    print(green_text + "Account created: " + emailStr + reset)
                    addAccount(emailStr, password, original)
                else:
